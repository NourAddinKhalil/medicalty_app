...
class InvoiceRepo extends GetxService with IRequestStructure {
  ...
  Future<List<InvoiceModel>> search(query) async {
    try {
      ...
      final list = GetBodyList.convertBodyToList<InvoiceModel>(
        res.body['data'],
        (element) {
          return InvoiceModel.fromMap(element);
        },
      );
  ...
  Future<InvoiceModel> getSingle(id) async {
    try {
      ...
      var employeeModel = InvoiceModel.defaultModel;
      employeeModel = InvoiceModel.fromMap(resBody);
  ...
  Future<InvoiceModel> add(model) async {
    try {
      ...
      final mod = InvoiceModel.fromMap(res.body['data']);
      return mod;
    } on CustomException catch (_) {
      rethrow;
    } on Exception catch (e) {
